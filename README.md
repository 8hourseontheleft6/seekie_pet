...
### v3.4.0 (2026-04-24) - 窗口避让+全屏移动
- **窗口检测**: 新增window_detector模块，使用Windows API枚举可见窗口
- **智能避让**: 移动路径上有窗口时自动换方向，不穿过窗口
- **全屏移动**: 被窗口挡住后自动扩展到全屏范围（0-100%），不限制右半边
- **动态路径规划**: 被窗口阻挡后自动寻找反方向可移动空间
- **性能优化**: 窗口检测结果缓存0.5秒，避免频繁枚举窗口
